...
prayerMedia.Feed = class {
  ...
  addPosts(posts) {
    ...
    for (let i = postIds.length - 1; i >= 0; i--) {
      ...
      const post = new prayerMedia.Post();
  ...
  toggleNextPageButton(nextPage) {
    ...
    if (nextPage) {
      ...
      prayerMedia.MaterialUtils.onEndScroll(100).then(loadMorePosts);
  ...
  showNewPosts() {
    ...
    for (let i = 0; i < postKeys.length; i++) {
      ...
      const postElement = new prayerMedia.Post();
  ...
  showGeneralFeed() {
    ...
    prayerMedia.firebase.getPosts().then(data => {
      // Listen for new posts.
      const latestPostId = Object.keys(data.entries)[Object.keys(data.entries).length - 1];
      prayerMedia.firebase.subscribeToGeneralFeed(
    ...
    prayerMedia.firebase.registerForPostsDeletion(postId => this.onPostDeleted(postId));
  }
  ...
  showHomeFeed() {
    ...
    if (this.auth.currentUser) {
      ...
      prayerMedia.firebase.updateHomeFeeds().then(() => {
        // Load initial batch of posts.
        prayerMedia.firebase.getHomeFeedPosts().then(data => {
          ...
          prayerMedia.firebase.subscribeToHomeFeed(
              (postId, postValue) => {
        ...
        prayerMedia.firebase.startHomeFeedLiveUpdaters();
...
prayerMedia.feed = new prayerMedia.Feed();
